...
class DeepQNetwork(object):
    ...
    def build_network(self):
        with tf.compat.v1.variable_scope(self.name):
            ...
            self.actions = tf.compat.v1.placeholder(tf.float32, shape=[None, self.n_actions], name='action_prise')
            self.q_target = tf.compat.v1.placeholder(tf.float32, shape=[None, self.n_actions])

            conv1 = tf.compat.v1.layers.conv2d(inputs=self.input, filters=32, kernel_size=(8, 8), strides=4,
                                               name='conv1',
                                               kernel_initializer=tf.compat.v1.variance_scaling_initializer(scale=2))
            conv1_activated = tf.nn.relu(conv1)

            conv2 = tf.compat.v1.layers.conv2d(inputs=conv1_activated, filters=64, kernel_size=(4, 4), strides=2,
                                               name='conv2',
                                               kernel_initializer=tf.compat.v1.variance_scaling_initializer(scale=2))
            conv2_activated = tf.nn.relu(conv2)

            conv3 = tf.compat.v1.layers.conv2d(inputs=conv2_activated, filters=128, kernel_size=(3, 3), strides=1,
                                               name='conv3',
                                               kernel_initializer=tf.compat.v1.variance_scaling_initializer(scale=2))
            conv3_activated = tf.nn.relu(conv3)

            flat = tf.compat.v1.layers.flatten(conv3_activated)
            dense1 = tf.compat.v1.layers.dense(flat, units=self.fcl_dims, activation=tf.nn.relu,
                                               kernel_initializer=tf.compat.v1.variance_scaling_initializer(scale=2))
...
class Agent(object):
    def __init__(self, alpha, gamma, mem_size, n_actions, epsilon, batch_size, replace_target=5000,
                 input_dims=(210, 160, 4), q_next='tmp/q_next', q_eval='tmps/q_eval'):
        self.n_actions = n_actions
        ...
        self.q_next = DeepQNetwork(alpha, n_actions, input_dims=input_dims, name='q_next', chkpt_dir=q_next)
        self.q_eval = DeepQNetwork(alpha, n_actions, input_dims=input_dims, name='q_next', chkpt_dir=q_eval)
    ...
    def store_transition(self, state, action, reward, state_, terminal):
        ...
        self.action_memory[index] = state_
    ...
    def learn(self):
        ...
        q_target[:, action_indices] = reward_batch + \
                                      self.gamma*np.max(q_next.copy(), axis=1)*terminal_batch
